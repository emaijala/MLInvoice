...
class InvoiceAction extends AbstractFormAction
{
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response, array $args = [])
    {
        ...
        if (!$this->id && ($templateId = $request->getQueryParams()['template'] ?? null)) {
            ...
            try {
                ...
                $this->updateFromInvoiceTemplate($invoiceTemplate);
    ...
    /**
     * Update the invoice from a template.
     *
     * @param Invoice $invoiceTemplate Template
     *
     * @return void
     */
    protected function updateFromInvoiceTemplate(Invoice $invoiceTemplate): void
    {
        assert($this->entity instanceof Invoice);
        $invoiceData = $invoiceTemplate->toArray();
        ...
        $paymentDays = $invoiceTemplate->getCompany()?->getPaymentDays()
        ...
        $this->entity->setTemplateInvoice($invoiceTemplate);
        $company = $this->entity->getCompany();

        foreach ($invoiceTemplate->getRows() as $templateRow) {
