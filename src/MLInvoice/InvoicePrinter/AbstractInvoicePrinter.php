 ...
 * @link     http://labs.fi/mlinvoice.eng.php
 */
abstract class AbstractInvoicePrinter
{
    protected $pdf = null;
    ...
    public function init(
        ...
        if (0 !== $invoiceId) {
            if (!($this->invoice = $this->invoiceRepository->getById($invoiceId))) {
                throw new \InvalidArgumentException("Could not find invoice with ID $invoiceId");
            }

            $invoiceData = $this->invoice->toArray(true);
            $recipientData = $invoiceData['company'];
            if ($recipientData) {
            ...
            if (!$invoiceData['base']) {
                if ($authenticated) {
                    throw new \Exception('Could not find invoice sender data');
                }
                return;
            }
            $senderData = $invoiceData['base'];
            ...
            if ($dateOverride) {
                $invoiceData['invoice_rows'] = array_filter(
                    $invoiceData['invoice_rows'],
                    function ($row) use ($dateOverride) {
                        return $row['row_date'] === $dateOverride;
                    }
                );
            }

            $invoiceRowData = $invoiceData['invoice_rows'];

            $invoiceTypeData = $invoiceData['type'];
            $this->attachments = [];
            foreach ($invoiceData['attachments'] ?? [] as $attachment) {
                if ($attachment['send']) {
        ...
        $this->translator->setActiveLanguage('non-default', $this->printLanguage);
        ...
        $this->separateStatement = ($this->printStyle == 'invoice')
            && $this->settingsManager->get('invoice_separate_statement') == 1;
        $this->allowSeparateStatement = ($this->printStyle == 'invoice')
            && $this->settingsManager->get('invoice_separate_statement') == 0;
        ...
        $this->senderAddressX = 10 + $this->settingsManager->get('invoice_address_x_offset', 0);
        $this->senderAddressY = 20 + $this->settingsManager->get('invoice_address_y_offset', 0);
        $this->recipientAddressX = 10 +
            $this->settingsManager->get('invoice_recipient_address_x_offset', 0);
        $this->recipientAddressY = 40 +
            $this->settingsManager->get('invoice_recipient_address_y_offset', 0);
        ...
        if (!$this->settingsManager->get('invoice_show_sequential_number')) {
            $this->columnDefs['sequence']['visible'] = false;
        }

        if (!$this->settingsManager->get('invoice_show_row_date')) {
        ...
        if ($this->settingsManager->get('invoice_row_description_first_line_only', false)) {
    ...
    protected function initPDF()
    {
        ...
        $pdf->markdown = $this->settingsManager->get('printout_markdown');
        $this->pdf = $pdf;
    }
    ...
    protected function printSender()
    {
        ...
        if (!isset($senderData['logo_filedata'])
            || $this->settingsManager->get('invoice_print_senders_logo_and_address')
        ) {
            $width = $this->settingsManager->get('invoice_address_max_width');
    ...
    protected function printRecipient()
    {
        ...
        if (!empty($contact['contact_person'])
            && $this->settingsManager->get('invoice_show_recipient_contact_person')
        ) {
            $pdf->setX($this->recipientAddressX);
        ...
        if (!empty($recipientData['email'])
            && $this->settingsManager->get('invoice_show_recipient_email')
        ) {
    ...
    protected function getInfoArray($bankInfo = false)
    {
        ...
        $data["{$locStr}Number"] = $this->getInvoiceNumber();
        ...
        if ($this->printStyle == 'invoice') {
            ...
            $data['PenaltyInterest'] = $this->formatNumber(
                $this->settingsManager->get('invoice_penalty_interest'), 1, true
            ) . ' %';
        ...
        if (($this->printStyle == 'invoice'
            && $this->settingsManager->get('invoice_show_delivery_info_in_invoice'))
            || ($this->printStyle == 'dispatch'
            && $this->settingsManager->get('dispatch_note_show_delivery_info'))
        ) {
        ...
        if ($this->printStyle == 'invoice') {
            if ($this->invoice->getRefundedInvoice()) {
                $data['RefundsInvoice'] = [
                    'value' => str_replace(
                        '%d',
                        $this->invoice->getRefundedInvoice()->getInvoiceNo(),
            ...
            $stateId = $this->invoice->getState()->getId();
            if ($stateId == 5) {
    ...
    protected function printSummary()
    {
        ...
        if ($this->senderData['vat_registered']
            && $this->settingsManager->get('invoice_show_vat_breakdown')
            && $this->groupedVATs
        ) {
        ...
        if ($this->senderData['vat_registered']) {
            ...
            if (!$this->settingsManager->get('invoice_show_vat_breakdown')) {
                $pdf->SetY($pdf->GetY() + 5);
                $pdf->Cell(
                    $right,
                    5,
    ...
    protected function printRow(&$pdf, $row)
    {
        ...
        if ($this->printStyle == 'dispatch'
            && $this->settingsManager->get('dispatch_note_show_barcodes')
    ...
    {
        $sequence = $this->settingsManager->get('invoice_show_sequential_number') == 1
    ...
    protected function getRowDescription($row)
    {
        ...
        default:
            if ($row['partial_payment']) {
                ...
                if ($this->settingsManager->get('invoice_display_product_codes')
                    && $row['product_code']
                ) {
    ...
    protected function printForm()
    {
        ...
        if ($this->settingsManager->get('invoice_show_info_in_form')
        ...
        if ($this->settingsManager->get('invoice_show_barcode') && $this->barcode) {
            $style = [
                'position' => '',
                'align' => 'C',
                'stretch' => true,
        ...
        foreach ($placeholders as $placeholder) {
            ...
            case 'invoice':
                ...
                case 'printout_type_caps':
                    ...
                    break;
                case 'pdf_link':
                    $url = $this->settingsManager->get('pdf_link_base_url');
                    if ($url) {
                        $language = $pcparts[2] ?? $this->printLanguage;
                        $uuid = $this->invoiceData['uuid'];
                        $ts = time();
                        $hash = $this->hmac->createHMAC(
                            [
                ...
                break;
            case 'config':
                $values[] = $this->settingsManager->get($pcparts[1]);
                break;
            case 'contact':
                $contact = $this->getContactPerson();
    ...
    protected function getTermsOfPayment($paymentDays)
    {
        if (!empty($this->recipientData['terms_of_payment'])) {
            ...
            $result = $this->settingsManager->get('invoice_terms_of_payment');
        }
    ...
    protected function getPeriodForComplaints()
    {
        if (!empty($this->senderData['period_for_complaints'])) {
            ...
            $result = $this->settingsManager->get('invoice_period_for_complaints');
        }
    ...
    {
        if (strpos($str, '::') > 0) {
            return $this->translator->translate($str, $placeholders, $default);
        }
        if ('invoice' !== $this->printStyle) {
            return $this->translator->translate(
                $this->printStyle . "::$str",
                $placeholders,
                $this->translator->translate("invoice::$str", $placeholders, $default)
            );
        }
        return $this->translator->translate("invoice::$str", $placeholders, $default);
    }
    ...
    protected function getRecipientNameAndAddress()
    {
        ...
        if (!$address || $this->printStyle != 'invoice'
            ...
            && !$this->settingsManager->get('invoice_send_reminder_to_invoicing_address'))
        ) {
    ...
    {
        return $this->invoice->getState()->getOffer() ? $this->invoice->getId() : $this->invoice->getInvoiceNo();
    }
}
